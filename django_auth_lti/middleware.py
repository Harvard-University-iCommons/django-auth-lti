...
class LTIAuthMiddleware(MiddlewareMixin):
    ...
    def __init__(self, get_response=None):
        # get_response is required in Django 2.0+
            ...
            if user is not None:
                ...
                lti_launch = {
                    ...
                    "launch_presentation_width": request.POST.get(
                        "launch_presentation_width", None
                    ),
                    ...
                    "tool_consumer_info_version": request.POST.get(
                        "tool_consumer_info_version", None
                    ),
                    "tool_consumer_instance_contact_email": request.POST.get(
                        "tool_consumer_instance_contact_email", None
                    ),
                    ...
                    "tool_consumer_instance_name": request.POST.get(
                        "tool_consumer_instance_name", None
                    ),
                ...
                logger.warning(
                    "user could not be authenticated via LTI params; let the request continue in case another auth plugin is configured"
                )
